...
这两个分数决定这条记忆会不会进入「用户画像」——一份长期常驻、每次对话都会读到的核心事实
清单。分高了画像会被琐事淹没，分低了 AI 永远认不出用户是谁。请按下面的固定顺序判断。

第一步，先判这条信息属于哪一类：
- A 类（长期）：身份背景、长期目标与方向、稳定偏好、重要关系、稳定的自我认知。
  判据：一年后再看这句话，对理解这个人依然成立且有用。
- B 类（具体）：会被回问的一次性事实（分数、日期、结果、决定）、当前正在持续的短期处境。
  判据：过几个月它可能已经过期或被新事实取代，但用户之后可能回头问起。
- C 类（琐事）：当下的心情、作息波动、一顿饭、一次晚起。过后无人再提 —— 不要产出 unit。

第二步，按类别给 importance：
- A 类里定义这个人是谁的（专业、身份、贯穿性的自我认知）：0.85
- A 类里的长期目标、稳定偏好、重要关系：0.7
- B 类：0.5
- 勉强值得一记、很快失效的：0.3
- 只有用户自己反复强调"这就是我"级别的事实才用 0.95

第三步，按证据质量给 confidence：
- 0.95：多条证据互相印证，或用户在不同场合反复说过。
- 0.85：用户在本批证据里明确、直接地说了这件事，且这个说法不依赖时效。
  只有一条证据也可以给 0.85——「明确说了」和「说过很多次」是两回事，不要因为只有一条
  证据就压到 0.7。
- 0.7：用户明确说了，但内容本身是一次性的或很快会变（今天的成绩、这周的安排）。
- 0.5：需要从字里行间推断才成立。
- 0.3：只是语气上的猜测。

confidence 和 importance 只能取 0.3 / 0.5 / 0.7 / 0.85 / 0.95，不要输出其他小数。
一条 A 类且用户明确自述的事实，正常结果是 importance≥0.7 且 confidence=0.85。
