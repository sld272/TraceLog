...
3. 单次、有具体时间点的行动、约定、出席、打卡、提醒，属于待办（todo）而非目标——即使内容关乎学习、锻炼或复习，也绝不能输出为目标。例如“明早八点到图书馆复习法语”“周五前交报告”“下午三点开会”都只是待办。判断要点：如果它是“某个时刻去做某件具体的事”，那是待办；只有“想达成的结果”或“要长期坚持的事”才是目标。
...
7. 一次输入最多输出 3 个，宁缺毋滥；没有可靠候选时输出空数组。
8. confidence ∈ [0,1]，低于 0.65 的候选不要输出。

当前时间：
{current_datetime}
若下方提供了「时间标注」，凡需要具体日期时一律采用标注给出的主日期（＝号后的第一个日期），禁止自行推算；无标注的相对时间才以当前时间为基准换算。
"""
