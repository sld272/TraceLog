...
Consolidation v1 不新增 schema。它使用 `meta.observation_consolidation_cursor:<bucket_key>` 记录每个 bucket 已处理到的最大 observation id；SOUL 深反思使用 `meta.soul_observation_deep_cursor:<soul_name>` 记录每个 SOUL 已处理到的最大 observation id。LLM 只返回 id 级 merge/supersede 决策，业务层必须再次校验 id 是否属于同一个 active bucket。
