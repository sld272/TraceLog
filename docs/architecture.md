...
尚未完成：私聊摘要沉淀、轻反思/深反思、导出和 Web/API 层。
...
| 全局深反思 | 可配置触发条件 + 用户手动触发 | 触发范围内所有 post 的轻反思聚合（不读 raw posts 原文） + 当前 user.md | reflection 文档 + user.md 条目级 patch（按章节 sensitivity 决定直落或入 pending） + relations 衰减/归一化 | 可配置 |
...
- [ ] 深反思最简版：前端手动触发，生成 reflection + user.md patch
