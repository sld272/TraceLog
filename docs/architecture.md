...
raw evidence（post / comment / comment_thread / chat_thread / todo / reflection）
    ...
    -> retrieval context（v1 注入 narrative 层；后续按权限与上下文预算渐进展开）
...
- 轻反思：读最近内容 → 抽取实体/情绪/事件/关系/公开 post observation → 写派生表。
- 深反思：按可配置触发策略聚合 observations → 生成 reflection + `user.md` / `soul_memories` 条目级 patch。
- `normal` 章节直落，`high` 章节使用更高阈值谨慎直落。
- 轻反思已扩展为 Signal Extraction：公开 post 抽完整结构化信号并写入 `global` observation；评论线程与私聊通过 cursor 增量提取 observation。
- 深反思之后追加 consolidation：已按 visibility boundary 执行 merge / supersede；promotion 由 deep reflection patch 完成。
...
5. **数据主权**：所有数据本地、可一键导出、可一键备份、可一键删除
6. **Observation 是中层记忆单位**：它不是 raw message，也不是深反思总结；它是即时蒸馏出的交互信号，必须带来源、时间、可见性和证据权限
7. **边界先于智能**：私聊和评论线程 observation 都属于当前 SOUL；评论线程只保留 post 作为来源上下文，不作为长期可见性边界。
8. **公开主链路 observation-first**：公开 post 回复优先使用 `global` observation 作为共享历史记忆信号，fanout 到单个 SOUL 时再追加该 SOUL 的 `soul_scoped` narrative；raw related posts 保留为 Chroma/FTS 的发现层和冷启动兜底，不再默认主导 prompt。
9. **用户只编辑最终长期记忆**：用户控制面作用于 `user.md` 和 `soul_memories/*.md`，不直接操作 observation、evidence、cursor 或 consolidation 状态。
10. **本地日志优先可调试**：JSONL 日志默认记录完整 LLM payload，并为 query rewrite、FTS、hybrid retrieval、memory retrieval 与上下文组装记录结构化 trace；密钥类字段继续自动脱敏。
...
| user.md 内部写入留痕 | `state.db` user_md_revisions 表 | ✗ | ProfileService 每次写入时记录；MemoryReviewService 提供只读审计 |
| SOUL 相处记忆历史 | `state.db` soul_memory_revisions 表 | ✗ | SoulMemoryService 每次写入时记录；MemoryReviewService 提供只读审计 |
...
| 实体、情绪、事件、关系、observation | `state.db` 派生表 | ✗ | 反思器 / ObservationExtractor |
...
- 当前无前端阶段已提供 `MemoryReviewService` 底座：未来前端可覆盖保存 `soul_memories/<name>.md`，并只读查看 `soul_memory_revisions`；这不会暴露该 SOUL 的 observation、evidence 权限或 consolidation 状态。
...
- 当前无前端阶段已提供 `MemoryReviewService` 底座：未来前端可覆盖保存整份 `user.md`，写入 `source='user'` 的 revision，并只读查看 `user_md_revisions`；用户编辑不经过 AI evidence/confidence gate。
...
- 评论线程不是长期可见性边界，只是当前 SOUL 关系中的发生场景。局部澄清留在 multi-message 上下文；跨场景值得记住的信号进入该 SOUL 的 `soul_scoped` 记忆。
- 公开 post 的共享上下文只召回 `global` observation；fanout 到单个 SOUL 时，可以追加该 SOUL 自己的 `soul_scoped` narrative。
- `source_soul_only` evidence excerpt 只能由对应 `scope_soul_name` 在私聊或评论线程中展开；公开 post 回复默认只使用 narrative，不展开私聊/评论原文。
...
| 层级 | 触发 | 输入 | 输出 | 频率 |
| --- | --- | --- | --- | --- |
| 轻反思 | 每条 post 写入后 | 当前 post + 最近 5 条 post + user.md「关键身份/关注的核心人际关系」两节作为已知实体词典 | entities / post_entities / emotions / events / posts.importance / relations 增量 + global observations | 每帖 |
| 全局深反思 | 可配置触发条件 + 用户手动触发 | 触发范围内 posts + active/global observations + 当前 user.md | reflection 文档 + user.md 条目级 patch（只允许 global observation 的 post evidence） | 可配置 |
| SOUL 记忆反思 | 全局深反思触发时，同步检查每个有新增 observation 的 SOUL | 当前 SOUL 的 soul_scoped observations + 当前 soul_memories/<name>.md | soul_memories/<name>.md 条目级 patch + soul_memory_revisions | 按 SOUL 独立触发 |

私聊与反思器的关系：

- **轻反思不读私聊**。每次私聊消息发送时不触发全局轻反思，避免噪声进派生表。
- **全局深反思不读私聊**。全局 `user.md` 只允许 `global` observation 的 post evidence 作为 promotion 证据，避免私聊里的玩笑、附和或情绪化表达污染共享画像。
...
| `observations[]` | `observations` + `observation_sources` | 固定 `visibility_scope=global`、`source_channel=post`、`evidence_access=all`；以 post_id 为幂等替换键 |
...
- `observations`：先删除该 post 由轻反思提取出的 `global` observation，再按本次输出重写，证据链固定指向 `source_type='post'` + `source_id=post_id`。
...
```
你是 TraceLog 的全局反思引擎。下面是本次触发范围内的 post 记录、
情绪标签、事件抽取、global observations，以及当前的 user.md（含 sensitivity 元数据）。
...
2. 对 user.md 的相关章节产出条目级 patch：
   ...
   - evidence 必须来自本次输入的 global observation sources；不得使用私聊或评论线程 observation
...
- scoped_observations: 当前 SOUL 的 soul_scoped observations
...
- 输出 patch 必须给 evidence：chat_message_id 或 comment_message_id，且必须来自输入 observation sources。
  ...
  "patches": [
    {
      ...
      "evidence": ["chat_message:392", "comment_message:18"],
      "confidence": 0.78
    }
  ]
}
...
Observation 是 raw evidence 与深反思之间的中层记忆单位。当前已完成数据底座，并已在公开 post、评论线程和私聊路径中写入 observation；Memory Retrieval v1 已把 observation 接入回复上下文，并支持 L1/L2 Progressive Disclosure。Deep Reflection 已消费 observation，Consolidation v1 已按边界桶整理 merge / supersede。第一版 observation 类型冻结为：
...
- 公开 post：已接入，抽取 entities / emotions / events / relations / observations / importance，其中 observation 固定写为 `global`、证据权限为 `all`。
- 评论线程：已接入，只抽取当前 SOUL 的 `soul_scoped` observations，`scope_post_id` 仅保留为来源上下文。
...
| 场景 | 允许召回 |
| --- | --- |
| 公开 post 共享上下文 | `global` |
| 公开 post 单 SOUL fanout | 共享上下文 + 当前 SOUL 的 `soul_scoped` |
| 私聊 | `global` + 当前 `scope_soul_name` 的 `soul_scoped` |
| 评论线程 | `global` + 当前 `scope_soul_name` 的 `soul_scoped` |

公开 post 的共享上下文永不召回 `soul_scoped` observation；fanout 到某个 SOUL 时只追加该 SOUL 自己的 `soul_scoped` narrative，不展开私聊/评论原文 excerpt。

公开 post 主链路已由 raw related posts 切换为 observation-first：

```
user.md
    -> # 相关记忆（global observations，FTS + post Chroma 间接语义召回）
    -> # 近期帖子（少量时间线连续感）
    -> # 相关帖子（仅当 # 相关记忆无命中时冷启动兜底）
    -> # 待办事项
```

因此，现有 post FTS/ChromaDB 仍然保留，但在公开回复中主要承担两类职责：先找出语义相关 `post_id`，再通过 `observation_sources(source_type='post')` 反查 `global` observation；以及在 observation 尚未积累时提供 raw post 兜底背景。
...
| --- | --- |
| `all` | 所有场景 |
| `source_soul_only` | 同一个 `scope_soul_name` 的私聊或评论线程 |
| `none` | 永不展开 |

因此，私聊与评论线程 evidence excerpt 永不出现在公开 post 回复上下文；公开 post fanout 只追加当前 SOUL 的记忆 narrative。

Consolidation 在 deep reflection 之后运行，用于整理 observation，而不是读取所有记忆后自由合并。v1 consolidation 本身只执行两类状态操作：

- `merge`：同一边界桶内重复 observation 合并，旧项标记为 `merged`。
- `supersede`：同一边界桶内新事实或新约定覆盖旧 observation，旧项标记为 `superseded`。

`promote` 不是 consolidation service 的直接操作。稳定、高置信、符合边界的 observation 通过 global / SOUL deep reflection 的 patch 流程进入 `user.md` 或 `soul_memories/<name>.md`；consolidation 只负责让后续检索与反思少读重复、过时或被覆盖的 observation。
...
- 同一 SOUL 的私聊与评论线程 observation 可以在同一个 `soul_scoped:<soul_name>` bucket 内整理。
- `soul_scoped` 永不 merge 到 `global`。
- `soul_scoped` 永不作为 `user.md` promotion evidence。
- `private_blocked` 不参与 consolidation。

Consolidation v1 不新增 schema。cursor 分为两类：`observation_cursors` 表只用于 Signal Extraction 的 source cursor，例如 chat/comment thread 已处理到哪条 message；`meta.observation_consolidation_cursor:<bucket_key>` 与 `meta.soul_observation_deep_cursor:<soul_name>` 是 process-level cursor，分别记录 consolidation bucket 与 SOUL 深反思处理到的最大 observation id。LLM 只返回 id 级 merge/supersede 决策，业务层必须再次校验 id 是否属于同一个 active bucket。
