...
8. **公开主链路 observation-first**：公开 post 回复优先使用 `global` observation 作为历史记忆信号；raw related posts 保留为 Chroma/FTS 的发现层和冷启动兜底，不再默认主导 prompt。
...
| SOUL 记忆反思 | 全局深反思触发时，同步检查每个有新增 observation 的 SOUL | 当前 SOUL 的 soul_scoped observations + 该 SOUL 参与的 post_visible observations + 当前 soul_memories/<name>.md | soul_memories/<name>.md 条目级 patch + soul_memory_revisions | 按 SOUL 独立触发 |
...
- scoped_observations: 当前 SOUL 的 soul_scoped observations，以及该 SOUL 参与的 post_visible observations
...
- 评论线程：已接入，只抽取 observations，并标记为 `post_visible`，`scope_post_id` 指向原始 post。
