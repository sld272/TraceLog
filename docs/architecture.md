...
- 轻反思已扩展为 Signal Extraction：公开 post 抽完整结构化信号并写入 `global` observation；评论线程与私聊后续只抽 observation。
...
| 实体、情绪、事件、关系、公开 post observation | `state.db` 派生表 | ✗ | 反思器 |
...
Observation 是 raw evidence 与深反思之间的中层记忆单位。当前已完成数据底座，并已在公开 post 轻反思中写入 `global` observation；评论线程、私聊、检索接管和 consolidation 仍是后续阶段。第一版 observation 类型冻结为：
