...
- `post_soul_orders` 记录每个公开 post 创建时启用 SOUL 的展示排序；首评实际生成完成时间不决定前端顺序，之后设置页调整 SOUL 排序也不改旧 post 的阅读顺序。
...
| SOUL 展示顺序快照 | `post_soul_orders` | 不注入 | RecordService / DB migration |
...
保存公开 post 时会同步写入当时启用 SOUL 的 `post_soul_orders` 快照。`reply_service.fanout()` 可以并发生成首评，但 timeline 和 post 详情读取 root comments 时按该 post 的快照排序；没有快照的旧数据才回退到评论创建时间和 id。
...
评论列表和 post 详情中，同一 post 下不同 SOUL 的 root comment / conversation 入口按 `post_soul_orders.sort_order` 固定排序；单个 SOUL 会话内部仍按 `comments.seq` 排序。
...
- `post_soul_orders` 只是展示顺序快照，不是语义证据，不进入 prompt、检索池或反思输入。

## 7. 模块边界

| 模块 | 职责 |
| --- | --- |
| `core/record_service.py` | 保存公开 post、写 SOUL 顺序快照、维护 pending vector doc、格式化 post |
...
| `core/comment_service.py` | `(post_id, soul_name)` 评论会话、按 post 快照列出会话入口、消息与回复上下文 |
...
| `core/db.py` | SQLite 初始化、轻量迁移/回填、事务、legacy cleanup、查询 helper |
