...
**滞回**防边界抖动：`ENTER = 0.82`，`EXIT = 0.62`（confidence 轴）。**core 准入强制 `DWELL = 2`**（连续两次对账稳定才迁入 md；`tier='core'` 单元首次出现不直接进画像，至少经一轮 confirm）——`tier` 由对账 LLM 自评，常驻 md 的身份地板必须有稳定性缓冲，防单轮误判污染。`in_md_slice` 物化结果，避免每次重算全表。
...
- `>= 0.70`（`MIN_IMPORTANCE`，本次由 0.60 上调）：核心身份/长期目标/核心偏好/重要关系，才够格进常驻画像（仍需同时满足 tier=core + confidence≥ENTER + DWELL）。
...
默认数值已按报告填入（ENTER 0.82 / EXIT 0.62 / DWELL 2、type 权重表、预算上限、迁移 confidence ≤0.75 与验收线、抽象率 ≥70%）。**实现中调整**：unit 存在门 `MIN_ADD_IMPORTANCE = 0.30`（reconciler，拒瞬时琐事 + prompt 回想价值测试）；核心画像门 `MIN_IMPORTANCE` 由 0.60 上调为 **0.70**（见 §3.2 三段分层）。
