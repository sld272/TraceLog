...
**滞回**防边界抖动：`ENTER = 0.82`，`EXIT = 0.62`（confidence 轴）。**实现中移除了按 op 计数的 `DWELL`**：它会把"只明确陈述一次的强身份"永久挡在画像外（永远凑不齐第二次 confirm），是设计缺陷。稳定护栏改由 `tier=core AND confidence≥ENTER AND importance≥MIN_IMPORTANCE` 三关共同承担——同时骗过三关的误判概率很低，且工作台可人工纠正。真正"跨多轮 reconcile 稳定"的缓冲应在 decay/重组阶段按"存活轮数"实现，而非 op 计数。`in_md_slice` 物化结果，避免每次重算全表。

**importance 三段分层**（importance 与 confidence 正交：前者"值不值得记"，后者"信不信为真"）：
- `< 0.30`（`MIN_ADD_IMPORTANCE`，写在 reconciler）：瞬时琐碎信息（"用户正在上课"），**对账 add 直接拒绝**，永不成为 unit。配合 reconcile prompt 的"回想价值测试"（这条以后回想对理解用户有用吗？无用即不产）软硬两道。
- `0.30 ~ 0.70`：有意义但非核心（近期状态等）——存为 unit、可被检索、可进"当前状态块"，但**不进** user.md。
- `>= 0.70`（`MIN_IMPORTANCE`，本次由 0.60 上调）：核心身份/长期目标/核心偏好/重要关系，才够格进常驻画像（仍需同时满足 tier=core + confidence≥ENTER）。
...
默认数值已按报告填入（ENTER 0.82 / EXIT 0.62、type 权重表、预算上限、迁移 confidence ≤0.75 与验收线、抽象率 ≥70%）。**实现中调整**：unit 存在门 `MIN_ADD_IMPORTANCE = 0.30`（reconciler，拒瞬时琐事 + prompt 回想价值测试）；核心画像门 `MIN_IMPORTANCE` 由 0.60 上调为 **0.70**（见 §3.2 三段分层）；并**移除按 op 计数的 DWELL**（改由 tier+confidence+importance 三关做稳定护栏，见 §3.2）。
