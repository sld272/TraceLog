...
│  souls/*.md  +  state.db.souls 表（启用/排序状态）              │
...
│  → 每个被调用的 SOUL 各自走一次 system prompt（人格段独立）     │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
...
│  读最近内容 → 抽取实体/情绪/事件 → 条目级 patch user.md         │
...
1. **会进 system prompt 的 → Markdown 文件**（`souls/*.md`、`user.md`）
   - 理由：每个 SOUL 文件整体加载，使用 prefix cache 友好
   - 理由：用户能直接看到自己的 AI 记忆，符合数据主权叙事
   - 注意：交互项目默认多 SOUL 同时启用，每条 post 会让所有启用 SOUL 各自跑一次调用，每次调用注入对应人格 + 共享的 user.md 与上下文
...
| --- | --- | --- |
| 用户群 | 开发者 | 普通学生/青年 |
| L1 人格 | `SOUL.md`（单文件，单一激活） | `souls/*.md`（多文件库，**默认多启用并发评论**；Agent 阶段可指定主 SOUL） |
...
- 反思器只读 `posts`，深反思可读最近一段 `chat_messages` 的 LLM 摘要（不读原文）。
...
3. 对每个启用 SOUL，按需读取并解析对应 `.md` 文件（frontmatter 仅作元数据，正文写入 system prompt 的 persona 段）
4. 加载结果可缓存在内存，文件 mtime 变化时重建

发帖时（详见 §6.1）：
- 对启用 SOUL 列表中的每一个，独立组装 system prompt（人格段不同，user.md 与检索上下文共享），并发发起一次 LLM 调用
...
这两张表也应在 §3.1 schema 中体现（迁移脚本一并建表）。
...
[2] ContextBuilder.build_context(user_input)
    ...
    │   - 读最近若干条 post（时间近邻）
    │   - 读活跃待办
    │   - 输出：persona-agnostic 共享上下文 + 启用 SOUL 列表
    │
    ▼
[3] ReplyService.fanout(post_id, shared_context, enabled_souls)
    │   并发对每个启用 SOUL 调用 LLM：
    │   ├─ system prompt = 该 SOUL 的人格段 + 共享上下文
...
[3] ChatService.call_chat_reply(soul, context, user_message)
    ...
    │   - 返回 reply + todos_to_upsert + todos_to_delete
    │   - 写一行到 chat_messages（role=assistant），source 字段
    │     在 todos 落盘时填 source_chat_message
    │
    ▼
...
| 进反思器 | 评论本身不进；evidence 仍是 posts | 轻反思不读；深反思读最近一段的 LLM 摘要（不读原文） |
...
| 场景 | 用什么 | 原因 |
| --- | --- | --- |
| 默认每次发帖 | 共享上下文（user.md + 检索）+ 每个启用 SOUL 各注入一次人格段 | 0 检索成本，仅多算 N 次 LLM |
| 私聊每次发消息 | 该 SOUL 人格 + user.md + 当前 thread 历史 + 对 posts 的 RRF 检索 + 该 SOUL 历史评论 | 中成本：检索一次，单次 LLM |
...
| 深反思 | 周/月周期 + 用户手动触发 | 周期内所有 post 的轻反思聚合（不读 raw posts 原文） + 周期内私聊摘要 + 当前 user.md | reflection 文档 + user.md 条目级 patch（按章节 sensitivity 决定直落或入 pending） + relations 衰减/归一化 | 每周/月 |

私聊与反思器的关系：

- **轻反思不读私聊**。每次私聊消息发送时不触发反思器，避免噪声进派生表。
- **深反思读私聊摘要而非原文**。深反思前先用便宜模型对周期内的私聊产出一段 ≤ 500 字的"用户主动谈了什么 / SOUL 观察到什么"摘要，再连同 posts 一起喂给深反思 LLM。原文不进 prompt，避免画像被对话寒暄拉偏。
- **私聊摘要写到 reflections 表**（type='chat_digest'），便于追溯哪份摘要影响了哪次画像更新。
...
情绪标签、事件抽取、私聊摘要，以及当前的 user.md（含 sensitivity 元数据）。
...
- [ ] 建立 `state.db` 与所有表（含 FTS5 双表、`souls` 表、`comments` 表、`user_md_revisions`、`pending_user_md_changes`）
...
- [ ] 异步轻反思（用 threading）
- [ ] 三因子重排
- [ ] user.md 增量 patch（第一期已通过 ProfileService.apply_patch 落地条目级 patch；可选项是把深反思全文重写改造为多 patch 输出）
...
- [ ] 深反思输出多 patch（替代第一期可能存在的全文 patch 简化版）
...
- [ ] 深反思：周期内私聊摘要 → reflections 表（type='chat_digest'）→ 进 user.md 画像更新输入
- [ ] 异步轻反思 + 失败重试队列
- [ ] 三因子重排上线
- [ ] 情绪曲线、实体提及频次、关系图可视化
...
| 私聊与主记忆 | 私聊独立存 `chat_threads` / `chat_messages`，不写 posts，不进 ChromaDB / FTS5；轻反思不读，深反思仅读 LLM 摘要；待办与 posts 共用同一张 todos 表 | 私聊噪声大，进检索池会污染语义结果；但用户在私聊里讲到的 deadline、任务仍属"用户意图"，必须落 todos |
...
| 深反思读私聊摘要导致画像偏移 | 私聊里的玩笑或附和被过度采纳 | 摘要 prompt 强调"区分用户事实 vs 闲聊"；深反思仍以 posts 为主证据；user.md 写入走 patch + revision，可一键回滚到任意历史版本；high 章节 AI 改动须经审核 |
