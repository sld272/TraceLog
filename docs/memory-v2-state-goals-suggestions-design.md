...
### 2.5 什么才值得记：回想价值门 + importance 三段分层

短期状态最容易滋生"瞬时琐事"型垃圾（"用户正在上课""刚吃完饭"）。它们往往 confidence 很高（确实为真）但 importance 很低（无价值）——所以判定要按 **importance** 而非 confidence,两者正交。

**软硬两道**：
- 软（reconcile prompt）：产出前过**回想价值测试**——"以后的对话里回想起这条,对理解用户有用吗?" 无用即不产,并给反例(上课/吃完饭/等公交)。
- 硬（reconciler 确定性门）：`add` 的 importance `< 0.30` 直接拒绝、记审计、cursor 照常推进。模型已稳定给琐事打低分,拿它自己的分做硬门,比指望它"别产出"可靠。
...
| unit 存在门 | `MIN_ADD_IMPORTANCE = 0.30` + 回想价值测试（软硬两道） |
| 核心画像门 | `MIN_IMPORTANCE = 0.70`（由 0.60 上调）；confidence 轴 ENTER 0.82 / EXIT 0.62 不变 |
