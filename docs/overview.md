...
- Observation 数据底座与提取器：已创建 `observations` / `observation_sources` / `observations_fts` / `observation_cursors`，公开 post 轻反思会写入 `global` observation，评论线程与私聊会通过 cursor 增量提取 `post_visible` / `soul_scoped` observation；尚未接入上下文检索。
...
- Observation 上层流程：尚未实现 Memory Retrieval、Progressive Disclosure 或 Consolidation。
...
公开 post 已会在轻反思阶段生成 `global` observation；评论线程和私聊会在 CLI 启动/退出时通过 cursor 增量生成 `post_visible` / `soul_scoped` observation。当前检索仍以公开 post 为中心，observation 尚未接管上下文检索。
...
- **Observation 记忆系统**：公开 post、评论线程与私聊 observation 已开始沉淀；后续用 FTS + 现有 post ChromaDB 的间接语义召回，不在第一版向量化所有 observation
