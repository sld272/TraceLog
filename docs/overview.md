...
- Observation 数据底座、提取器、Memory Retrieval v1、Progressive Disclosure v1 与 Consolidation v1：已创建 `observations` / `observation_sources` / `observations_fts` / `observation_cursors`，公开 post 轻反思会写入 `global` observation，评论线程与私聊会通过 cursor 增量提取 `post_visible` / `soul_scoped` observation；回复上下文已接入 L1 narrative 召回，并按权限展开少量 L2 evidence excerpt；深反思已消费 observation，consolidation 已按 visibility boundary 执行 merge / supersede。
...
- **Observation 记忆系统**：公开 post、评论线程与私聊 observation 已沉淀并进入 Memory Retrieval v1 + Progressive Disclosure v1 + Consolidation v1；后续可补 aging/archive 与观察向量化
