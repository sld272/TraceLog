...
当前 Web + CLI 双入口原型的核心闭环已基本跑通。默认入口是 Web；图片附件、图片识别、网页搜索、后台任务事件流和记忆工作台主要在 Web/API 路径中使用，CLI 保留文本公开记录、SOUL 管理、私聊、评论线程、工具开关与退出反思能力。核心 service 落位、SQLite + FTS5 + ChromaDB 三轨数据底座、SOUL 与私聊/评论双通道、轻反思与全局/SOUL 深反思、TodoTool 开关全部可用。
...
- Web 应用：FastAPI API + Vite/React 前端覆盖首页 timeline、发帖、SOUL 首评、基于 `(post_id, soul_name)` 的内联评论对话、待办、反思预览/触发、SOUL 私聊、右侧摘要栏、设置页和长期记忆工作台。
- 记录与检索：双写 SQLite + ChromaDB，Chroma collection 按 embedding 模型与 base URL 指纹隔离，支持 `pending_vector_doc` 重索引；FTS5 双 tokenizer + ChromaDB + 动态权重 hybrid scoring 融合。
...
- 回复上下文：公开 post 注入 `user.md`、hybrid search 命中的 raw `# 相关帖子`、活跃 todos 和可选网页搜索结果；私聊/评论对话使用当前消息序列 + unified retrieval 命中的 `# 相关记忆`。
...
- 画像与长期记忆审计：sensitivity 阈值分级落盘，写 `user_md_revisions` / `soul_memory_revisions` 留痕；Web 记忆工作台已支持结构化或 Markdown 全文编辑 `user.md` / `soul_memories`，revision 浏览 API 已有。
- API 后台任务：Web/API 公开 post pipeline 使用 SQLite `jobs` 表和单 worker 队列串行领取任务；多 SOUL 首评仍在 `reply_service.fanout()` 内部并发。手动全局/SOUL 深反思也复用 `jobs`。
- 测试：当前测试套件可用 `python -m unittest discover -q` 运行，覆盖 API 管理、chat / comment / profile / reflector / todo / soul / cli / vectorstore / workspace / vision / web search。

**尚未完成 / 公开前后优先级**

- 首次配置体验：无 `config.json` 时 Web/API 也能启动并进入设置页；保存模型配置后会尝试热加载后端运行时。
- 记忆审计前端：长期记忆编辑 Web UI 已完成基础版；revision 浏览、diff 和回滚 UI 尚未完成。
...
Web Composer / API request
  ...
  → JobWorker 串行领取 jobs
      → vectorstore.index_post                  # 正文 upsert ChromaDB；失败进入 pending_vector_doc
      → vision_service.describe_attachments     # 可选：图片摘要 → vision_cache + post_vision 向量文档
      → reply_context.rewrite + retrieval.hybrid_search
      → context_builder.build_context           # user.md + raw 相关帖 + 活跃 todos + 可选网页搜索
...
**L3：结构化证据层** — 存储所有历史数据（`state.db` SQLite + `chroma_db/` 向量索引）。posts、comments、chat messages、attachments、vision_cache、entities、emotions、events、todos、reflections 等全部落 SQLite；语义向量索引 posts、公开 post 图片摘要、comments 和 chat messages；关键词查询只走 posts 的 FTS5 双表。
