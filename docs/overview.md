...
- Observation 数据底座与公开 post 提取：已创建 `observations` / `observation_sources` / `observations_fts` / `observation_cursors`，公开 post 轻反思会写入 `global` observation，并支持边界字段、FTS、cursor 与证据清理；尚未接入评论线程/私聊提取器和上下文检索。
...
- Observation 上层流程：尚未实现评论线程/私聊 Signal Extraction、Memory Retrieval、Progressive Disclosure 或 Consolidation。
...
公开 post 已会在轻反思阶段生成 `global` observation；当前检索仍以公开 post 为中心，observation 尚未接管上下文检索。私聊和评论线程还不会自动生成 observation。
...
- **Observation 记忆系统**：公开 post observation 已开始沉淀；后续补齐评论线程/私聊 Signal Extraction，并用 FTS + 现有 post ChromaDB 的间接语义召回，不在第一版向量化所有 observation
