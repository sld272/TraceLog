...
- 记录与检索：双写 SQLite + ChromaDB，支持 `pending_embedding` / `pending_vector_doc` 重索引；FTS5 双 tokenizer + ChromaDB + 动态权重 hybrid scoring 融合。
...
- 首次配置体验：无 `config.json` 时 API 仍需 CLI 向导或手动配置后才能完整启动，Web 还不能在未配置状态下单独进入配置页。
