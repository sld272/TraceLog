...
当前 CLI 原型的闭环已基本跑通。核心 service 落位、SQLite + FTS5 + ChromaDB 三轨数据底座、SOUL 与私聊/评论双通道、轻反思与全局/SOUL 深反思、TodoTool 开关全部可用。
...
- Observation 数据底座、提取器、Memory Retrieval v1、Progressive Disclosure v1 与 Consolidation v1：已创建 `observations` / `observation_sources` / `observations_fts` / `observation_cursors`，公开 post 轻反思会写入 `global` observation，评论线程与私聊会通过 cursor 增量提取当前 SOUL 的 `soul_scoped` observation；公开 post 回复主链路已 observation-first，优先注入 `# 相关记忆`，并按权限展开少量 L2 evidence excerpt；深反思已消费 observation，consolidation 已按 visibility boundary 执行 merge / supersede。
- 待办：TodoTool 严格只从公开 post 抽取，支持开关控制。
- 反思器：每帖轻反思（含 pending 重试）、CLI 退出时全局深反思、按 SOUL observation 游标的 SOUL 深反思，以及退出时 observation consolidation。
- 日志：本地 JSONL 日志默认记录完整 LLM prompt/response/parsed 结果，并为 query rewrite、FTS 构造、hybrid retrieval、observation memory retrieval 和 context assembly 写结构化 debug trace；API key/token 仍会自动脱敏。
...
- 记忆前端：长期记忆编辑与 revision 审计已有 service 底座，但尚未接入 API / Web UI。
- Observation 后续增强：尚未实现 entity resolution、自动 aging/archive、observation 向量索引。
...
read_cli_input
  ...
  → context_builder.build_context               # user.md + 相关记忆 + 近期帖子 + raw 相关帖兜底 + 活跃 todos
  ...
  → CLI 退出
      → reflector.trigger_global_deep_reflection
        → 读取 global observations → 写 reflections + profile_service.apply_patch（normal / high 双阈值）
      → reflector.trigger_pending_soul_deep_reflections
        → 每个 SOUL 读自己的 soul_scoped observations → soul_memories/<name>.md + revision
      → observation_consolidation.run_observation_consolidation
        → 按 visibility boundary 做 merge / supersede
```
...
公开 post 已会在轻反思阶段生成 `global` observation；评论线程和私聊会在 CLI 启动/退出时通过 cursor 增量生成当前 SOUL 的 `soul_scoped` observation。Memory Retrieval v1 会把允许边界内的 observation narrative 注入 `# 相关记忆`；在公开 post 回复里，`# 相关记忆` 已接管共享主链路，raw `# 相关帖子` 只在没有相关记忆命中时作为冷启动兜底，每个 SOUL 还会追加自己的 `soul_scoped` 记忆上下文。Progressive Disclosure v1 只在权限允许时展开少量 `observation_sources.excerpt`，不读取完整原文；公开 post 回复可使用当前 SOUL 的私有 narrative，但默认不展开私聊/评论原文 excerpt。Deep Reflection 已把 observation 作为 promotion 证据来源，Consolidation v1 会在同一边界桶内整理重复或被新约定覆盖的 observation。

用户可控面当前收敛在最终长期记忆层：用户编辑 `user.md` 或 `soul_memories/<name>.md` 时会以 `source='user'` 写入 revision；observation、evidence、cursor 与 consolidation 状态仍是内部工作层，不暴露为用户直接操作对象。
...
**反思器** — 后台异步 LLM agent。轻反思每条 post 后抽取实体/情绪/事件和公开 post observation；深反思按可配置触发策略聚合 observation，生成 reflection 并更新 `user.md` / `soul_memories`；consolidation 在同一 visibility boundary 内整理 observation 状态。
...
| 用户记忆控制面 | 只编辑 `user.md` / `soul_memories` 最终长期记忆 | 用户能修正 AI 最终记住的内容，但不需要理解或破坏 observation 内部整理流程 |

## 6. 后续方向

TraceLog 还能基于本架构生长出：

- **Observation 记忆系统**：公开 post、评论线程与私聊 observation 已沉淀并进入 Memory Retrieval v1 + Progressive Disclosure v1 + Consolidation v1，公开 post 主链路已 observation-first；后续可补 aging/archive 与观察向量化
- **Entity Resolution**：实体重复、实体冲突和关系消歧仍是独立后续阶段，不混入 observation consolidation
