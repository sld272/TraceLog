...
- 首次配置体验：无 `config.json` 时 Web/API 也能启动并进入设置页；保存模型配置后应用会自动重新加载配置，失败时继续使用上一次可用配置。
...
**L3：结构化证据层** — 存储所有历史数据（`state.db` SQLite + `chroma_db/` 向量索引）。posts、comments、chat messages、attachments、vision_cache、entities、emotions、events、todos、reflections 等全部落 SQLite；SQLite 同时保存向量文档账本与 outbox，ChromaDB 只保存可重建的派生向量；语义向量索引 posts、公开 post 图片摘要、comments 和 chat messages；关键词查询只走 posts 的 FTS5 双表。
