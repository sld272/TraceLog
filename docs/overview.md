...
- Observation 数据底座：已创建 `observations` / `observation_sources` / `observations_fts` / `observation_cursors`，支持边界字段、FTS、cursor 与证据清理；尚未接入提取器和上下文检索。
...
- Observation 上层流程：尚未实现 Signal Extraction、Memory Retrieval、Progressive Disclosure 或 Consolidation。
...
Observation 数据底座已存在，但当前检索仍以公开 post 为中心；私聊和评论线程还不会自动生成 observation，也不会进入上下文检索。
...
**反思器** — 后台异步 LLM agent。轻反思每条 post 后抽取实体/情绪/事件；深反思按可配置触发策略聚合，生成 reflection 并更新 `user.md`。
...
- **Observation 记忆系统**：在 raw evidence 与深反思之间加入中层信号，先用 FTS + 现有 post ChromaDB 的间接语义召回，不在第一版向量化所有 observation
