...
- 数据底座：`schema.sql` 唯一 SQLite 初始化脚本，WAL、外键、FTS5/trigram 校验；旧版中间层表会在 `init_db()` 时清理。
...
- Web 应用：FastAPI API + Vite/React 前端覆盖首页 timeline、发帖、SOUL 首评、内联评论线程、待办、反思预览/触发、SOUL 私聊和右侧摘要栏。
...
- 回复上下文：公开 post 只注入 `user.md`、hybrid search 命中的 raw `# 相关帖子` 和活跃 todos；不再注入 `# 近期帖子` 或 `# 相关记忆`。私聊/评论线程使用当前线程消息 + raw 相关帖子 + 该 SOUL 对相关帖子的历史评论。
- 反思器：每帖轻反思只维护实体、情绪、事件、关系和 post importance；全局深反思直接读取 raw posts + 当前 `user.md` + todos，对画像做 confirm / revise / retract / add；SOUL 深反思直接读取该 SOUL 的 raw chat/comment thread messages，更新对应 `soul_memories/<name>.md`。
...
- Web 设置界面：当前首次配置仍需要 CLI 向导或手动 `config.json`，后续应在 Web 设置里完成 provider、模型、embedding 和日志配置。
...
Web Composer / CLI input
  ...
  → vectorstore.index_post                      # upsert ChromaDB；失败进入 pending_embedding
  ...
  → 手动触发或 CLI 退出
      ...
      → reflector.trigger_soul_deep_reflections
        → 每个 SOUL 读取自己的 raw thread messages → soul_memories/<name>.md + revision
```

SOUL 私聊链路：按 thread 加载历史，检索 raw posts/相关评论作为背景，调用单 SOUL，写 `chat_messages`。私聊不写 posts，不进 FTS5/ChromaDB，不触发轻反思；其长期影响只可能在 SOUL 深反思中进入该 SOUL 自己的相处记忆。

评论线程链路：与私聊对称，但 thread 与 root comment 绑定到 `comment_threads`，由 SOUL 深反思统一吸收。Web 中评论线程以内联回复形式长在 SOUL 首评下方。
...
**L3：结构化证据层** — 存储所有历史数据（`state.db` SQLite + `chroma_db/` 向量索引）。posts、comments、thread messages、entities、emotions、events、todos、reflections 等全部落 SQLite；语义向量仅索引 posts；关键词查询走 FTS5 双表，混合查询走 RRF 融合。
