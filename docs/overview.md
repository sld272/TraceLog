...
- Observation 记忆系统：当前仍是设计冻结阶段，尚未创建 observation 表、提取器、统一检索或 consolidation 流程。
...
Observation 系统尚未实现。当前检索仍以公开 post 为中心；私聊和评论线程不会生成可检索 observation，也不会进入 FTS5 / ChromaDB。
