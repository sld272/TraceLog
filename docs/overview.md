...
- SOUL 体系：同步、列表、新建/编辑、启用/禁用、排序、缺失文件自动禁用；默认初始化“默认”与“毒舌好友”两个 SOUL。每个公开 post 会保存发帖当时的 SOUL 排序快照，旧 post 的首评展示不随之后的设置排序变化。
...
评论对话链路：与私聊相似，但会话键就是 `(post_id, soul_name)`；`comments.seq=0` 是首评，后续追问/回复以内联形式长在 SOUL 首评下方，并由 SOUL 深反思统一吸收。同一 post 下不同 SOUL 的首评和会话入口按 `post_soul_orders` 的发帖时快照排列，单个 SOUL 会话内部仍按 `seq` 排。
...
**L3：结构化证据层** — 存储所有历史数据（`state.db` SQLite + `chroma_db/` 向量索引）。posts、post_soul_orders、comments、chat messages、attachments、vision_cache、entities、emotions、events、todos、reflections 等全部落 SQLite；SQLite 同时保存向量文档账本与 outbox，ChromaDB 只保存可重建的派生向量；语义向量索引 posts、公开 post 图片摘要、comments 和 chat messages；关键词查询只走 posts 的 FTS5 双表。
