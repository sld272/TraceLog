...
① reconcile 对账 → ② relink 证据重挂（evidence↔unit，用户编辑后）→ ③ reflect
确定性反思（无 LLM：state 衰减 + contextual→core 晋升，本轮 owner 即时跑 +
每日一次全量扫兜底，meta 门控——不活跃 owner 的过期 state 也要能淡出）→
④ 墓碑断言回填 → ⑤ consolidate 巩固（LLM 去重去矛盾）→ ⑥ crosslink 跨桶链接
→ job 尾刷新视图/向量索引。命名约定：reflect 与 consolidate 是两个独立 pass
（旧文档的"深反思 P2a/P2b"合称废止），consolidate 的 op actor 记为
`consolidation`；crosslink 管 unit↔unit 跨桶关系，relink 管 evidence↔unit 重挂。

**不挡交互**是调度铁律，三道保护：job 认领交互优先（reconcile 只在没有其他
pending job 时被认领）；桶间让路（`should_yield` 在每桶之间轮询，有交互 job
等待就提前收工、跳过尾部维护，续跑 job 在队列排空后无损接续——桶游标保证不丢
证据）；consolidate 开跑前单独再让路一次。

consolidate 是最贵的 pass（owner 全量 active units 进一个 prompt，判错用户可见），
自动触发三重闸：active units ≥ 12、距该 owner 上次巩固 ≥ 3 天冷却、自上次巩固后
有 unit 变更；每次 job 至多巩固 1 个最欠账 owner（从未巩固者优先）。冷却戳只在
成功后写入，失败的 owner 留在欠账队列重试。
...
## 跨桶链接 crosslink（P1：链接，不合并）

桶是隐私底座，consolidate/supersede 拒绝跨桶——但同一事实/矛盾常落在两个桶里。
低频 crosslink pass（`memory_crosslink`，管 unit↔unit 的跨桶关系；不要和管
evidence↔unit 重挂的 relink 混淆）搭 reconcile 尾部的车：最近变动的 units → 跨桶候选（同文精确匹配 +
...
闭环由 crosslink 的链接维护完成：任一端 last_confirmed 越过链接时间（回访回答被
