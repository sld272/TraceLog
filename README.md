...
- **评论线程**：每条 SOUL 首评下面都可以继续多轮回复，消息写入 `comment_threads` / `comment_messages`；线程不进入公开 post 检索池，但会进入该 SOUL 的深反思材料。
...
- **SOUL 文件管理**：首次初始化会创建默认 SOUL 文件；你也可以通过 CLI 创建、启用、禁用、排序，或手动编辑 `workspace/souls/*.md` 后 `/soul resync`。
...
当前 Web 设置界面尚未完成。第一次使用前请先用 CLI 完成配置：
...
- Web 设置界面尚未完成；第一次使用前需要通过 CLI 或手动 `config.json` 完成配置。
