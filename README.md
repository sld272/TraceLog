...
TraceLog 是一个本地优先的个人成长 AI 伴侣。你可以像发动态一样写下日常碎片，AI 会回应你、帮你提取待办，并把记录沉淀成可检索、可迁移、可由你掌控的个人记忆。
...
即可退出；退出时 TraceLog 会整理近期记录并更新 `workspace/user.md`。
...
- 当前回复仍是单 AI 回复；多 SOUL 并发评论的数据地基已经初始化，但回复流程尚未接入。
- `user.md` 现在仍由退出时的画像整理整体覆盖，条目级 patch 和审核区还未实现。
- 旧版 `workspace/posts/*.md`、`profile.md`、`todos.json` 不会自动迁移到 `state.db`。
