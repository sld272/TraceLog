...
- **图片附件**：公开 post、评论追问和 SOUL 私聊都支持本地 JPEG/PNG 图片附件；图片写入 `workspace/attachments/`，后端会自动旋转方向并清理 EXIF。
...
- 图片附件当前限制为单次最多 9 张、单张 5MB 以内，仅支持 JPEG/PNG；识图、缩略图生成和附件删除 UI 尚未实现。
