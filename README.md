...
- **退出深反思**：退出 CLI 时会对上次深反思后的公开记录生成一次全局深反思，并保存到 `reflections` 表。
...
即可退出；退出时 TraceLog 会触发一次全局深反思。当前版本只保存 reflection，不会自动覆盖 `workspace/user.md`。
...
- 深反思现在会保存到 `reflections` 表，但还不会生成和应用 `user.md` 条目级 patch。
