...
TraceLog 是一个本地优先的个人成长 AI 伴侣。你可以像发动态一样写下日常碎片，AI 会回应你、帮你提取待办，并把记录沉淀成可检索、可导出、可由你掌控的个人记忆。
...
- **画像 Patch**：深反思会生成 `user.md` patch，normal 章节自动落盘，high 章节进入待审核区。
...
即可退出；退出时 TraceLog 会触发一次全局深反思，并把可靠的 normal 画像变更写入 `workspace/user.md`。
...
- high sensitivity 的画像变更现在只会进入待审核区，CLI 只支持 `/profile pending` 查看，还不能采纳/拒绝。
...
- `core/profile_service.py`：`user.md` patch、revision 与 pending 管理
