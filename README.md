...
建议使用 Python 3.11。本仓库开发与本地运行默认使用 conda 环境 `tracelog`；如果已经 `conda activate tracelog`，下面命令里的 `conda run -n tracelog` 可以省略。
...
API 启动时会读取 `config.json`。如果是第一次使用、配置文件还不存在，Web 仍会启动，并自动引导你进入设置页配置主模型和 Embedding；保存后应用会自动重新加载配置，重新加载失败时会继续使用上一次可用配置并在设置页提示。CLI 向导仍可作为备用配置路径：
...
```bash
conda run -n tracelog python main.py web --backend-port 8001 --frontend-port 5174
```
