...
- **多 SOUL 评论**：所有启用的 SOUL 会并发回复公开记录，首条回复写入 `comments`，展示排序由发帖当时的 SOUL 顺序快照控制。
...
- Web 设置页已支持模型、Embedding、图片识别、网页搜索、日志、SOUL、长期记忆编辑和本地数据状态；无配置文件时也能启动 Web，并在保存模型配置后自动重新加载应用配置。
