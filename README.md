...
API 启动时会先读取 `config.json`。如果是第一次使用、配置文件还不存在，请先用 CLI 向导生成基础配置，或手动创建 `config.json`；应用能启动后，可以在 Web 设置页继续维护主模型、Embedding、图片识别、网页搜索、日志、SOUL 和长期记忆配置：
...
- Web 设置页已支持模型、Embedding、图片识别、网页搜索、日志、SOUL、长期记忆编辑和本地数据状态；但无配置文件时 API 仍需先通过 CLI 或手动 `config.json` 完成首次启动配置。
