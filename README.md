...
TraceLog 是一个本地优先的个人成长 AI 伴侣。你可以像在任意社交媒体发动态一样写下日常碎片，让多个 SOUL 以不同人格回应你；系统会把公开记录、待办、反思、画像和 SOUL 相处记忆沉淀到本地 `workspace/`，并用 SQLite + ChromaDB 支撑检索与后续反思。

当前版本提供 Web + CLI 双入口。默认入口是 Web，完整的图片附件、图片识别、网页搜索、后台任务事件流和记忆工作台主要在 Web/API 路径中使用；CLI 提供基础功能。

更多文档：项目概述见 [docs/overview.md](docs/overview.md)，技术架构见 [docs/architecture.md](docs/architecture.md)，数据库设计见 [docs/database.md](docs/database.md)，API 说明见 [docs/api.md](docs/api.md)。
...
- **多 SOUL 评论**：所有启用的 SOUL 会并发回复公开记录，首条回复写入 `comments`；每个 post 会保存发帖当时的 SOUL 顺序快照，之后设置页改排序只影响新 post。
...
- **TodoTool 与建议确认**：Todo 是可开关工具，默认维持公开 post 自动写入；设置 `TODO_SUGGESTIONS_ENABLED=1` 后，public/comment/chat 都只生成待办建议，用户在回复下或建议托盘采纳后才改 `todos`。
- **GoalTool**：目标拥有独立的 `goals` 真相源和 Web 管理页，支持长/短期、进行中/暂停/完成/放弃、当前关注与推进记录；`GOAL_SUGGESTIONS_ENABLED=1` 时，三路回复会在对话当下询问是否正式追踪目标。
...
- **本地 JSONL 日志**：运行日志写入 `workspace/logs/current.jsonl`；每次启动会把上一次 current 轮转到 `workspace/logs/history/`，默认最多保留 100 个历史日志，便于长期使用后回放检索与回复质量。
- **记忆工作台与整理记录**：Web 设置页可以结构化或以 Markdown 全文编辑 `user.md` 和 `soul_memories/<name>.md`，写入 revision 审计；反思页会展示最近整理产出、变更摘要和完整 revision 快照。
- **SOUL 文件管理**：首次初始化会创建 `拾迹者`、`温柔树洞`、`毒舌好友` 三个默认 SOUL 文件；你可以在 Web 设置里用 AI 生成或手写 Markdown 新建 SOUL、启用/禁用和排序，也可以通过 CLI 管理，或手动编辑 `workspace/souls/*.md` 后 `/soul resync`。
- **记忆系统 v2（实验，默认关闭）**：事件驱动的新记忆引擎，把 raw evidence → memory units → 画像视图组成单向 DAG，用 reconcile 替换 light/deep 反思，回复时分层注入（基线画像 / 当前状态 / 相关记忆 / 尚未整理的近期动态），并实现"人格不串戏"边界（公开跨人格共享、私聊各自隔离、自己的私密靠提示词自审）。由两个环境变量开启且默认 legacy 零影响：`MEMORY_V2_WRITE_MODE=reconcile`、`MEMORY_V2_READ_MODE=units|units_and_freshness`。架构与已实现状态见 [`docs/memory-v2-architecture.md`](docs/memory-v2-architecture.md)。
...
TraceLog 使用 OpenAI SDK 调用主 LLM，因此主模型 provider 需要兼容 OpenAI Chat Completions 风格接口。ChromaDB 的 embedding function 会使用单独的 embedding model/base URL/key 配置，并按 embedding 模型与 base URL 派生独立 collection，避免不同 embedding 空间混用。

API 启动时会读取 `config.json`。如果是第一次使用、配置文件还不存在，Web 仍会启动，并自动引导你进入设置页进行配置；保存后应用会自动重新加载配置，重新加载失败时会继续使用上一次可用配置并在设置页提示。CLI 向导仍可作为备用配置路径：
...
```bash
python main.py web --backend-port 8000 --frontend-port 5173
```
...
1. 保存公开 post 到 SQLite，写入发帖当时的 SOUL 顺序快照，并通过 trigger 同步 FTS5 索引。
...
7. `context_builder.build_context()` 组装共享上下文：`user.md`、goaltool 长期目标/当前关注、raw 相关 posts、活跃 todos，以及 Web/API 路径上的可选网页搜索结果。
8. `reply_service.fanout()` 让启用的 SOUL 并发生成首条评论；每个 SOUL 的 soul Markdown 和 `soul_memories/<name>.md` 在对应 SOUL 调用时注入，前端展示按该 post 的 SOUL 顺序快照排列。
9. TodoTool job 抽取或更新明确待办；启用 `TODO_SUGGESTIONS_ENABLED` 时只写 pending suggestion。回复 job 也可按开关生成 goal/todo 建议并挂到回复下确认。轻反思 job 写入 entities / emotions / events / relations / importance；累计到阈值后可触发全局深反思 job。

私聊和评论线程使用独立消息表与原生 multi-message 历史，不会保存为公开 post，也不会触发后台 TodoTool job 或轻反思；启用建议开关时，回复后可直接产生 goal/todo 建议。图片附件会保存和展示；识图可用时当前图片会被摘要后注入 LLM 上下文，识图不可用时只注入“有图片但不能查看内容”的边界提示。

## 设计理念

TraceLog 的长期方向不是做一个普通聊天机器人，而是做一个由你掌控的个人记忆系统：

- 可以由你直接阅读和修改的长期信息，优先保存在 Markdown 中。
...
- Web 设置页已支持模型、Embedding、图片识别、网页搜索、日志、SOUL、长期记忆编辑、本地数据状态，以及向量索引重试/对账；无配置文件时也能启动 Web，并在保存模型配置后自动重新加载应用配置。
...
- 长期记忆编辑和 revision 浏览已有 Web 工作台；revision 回滚 UI 尚未完成。
